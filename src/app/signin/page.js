...
function SignInPageContent() {
  ...
  // All your existing state and logic goes here
  const [isLogin, setIsLogin] = useState(true);
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  const [loading, setLoading] = useState(false);
    ...
    <Suspense fallback={
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-white dark:from-gray-900 dark:to-gray-950">
        <div className="text-center">
          ...
          <h3 className="text-xl font-semibold text-slate-800 dark:text-white">Loading...</h3>
          <p className="text-slate-600 dark:text-gray-400 mt-2">Please wait</p>
        </div>
      </div>
    }>
      <SignInPageContent />
